...
class cpu:
    ...
    def process(self, resource, packet):
        with resource.request() as req:
            ...
            data.record.write(f"before release resource now: {resource.count}\n")
            resource.release(req)
            data.record.write(f"finish packet: {packet.packetID}\n")
            data.record.write(f"resource now: {resource.count}\n")
            ...
            if len(self.node.queue) > 0:
                ...
                self.env.process(self.process(resource, nextPacket))
